...
get_ocp_node_type(){
    ...
    HNAME=$(cat ${CITELLUS_ROOT}/hostname)

    NODELISTFILELIST=$(find ${CITELLUS_ROOT}/../../ -name *_all_nodes.out 2>/dev/null)
...
calculate_cluster_pod_capacity(){
    ...
    for nodes in ${CLUSTERNODELIST}; do
        if [ -d ${nodes}/sosreport-*/sos_commands ]; then
            ...
            if [ -e ${nodes}/sosreport-*/etc/origin/node/node-config.yaml ] ;then
            ...
            elif [ -e ${nodes}/sosreport-*/../etc/origin/node/node-config.yaml ] ;then
                XXX=$(grep 'pods-per-core:' -A1 ${nodes}/sosreport-*/../etc/origin/node/node-config.yaml)
                ZZZ=$(grep 'max-pods:' -A1 ${nodes}/sosreport-*/../etc/origin/node/node-config.yaml)
            elif [ -e ${nodes}/sosreport-*/../tmp/node-config.yaml ] ;then
                XXX=$(grep 'pods-per-core:' -A1 ${nodes}/sosreport-*/../tmp/node-config.yaml)
                ZZZ=$(grep 'max-pods:' -A1 ${nodes}/sosreport-*/../tmp/node-config.yaml)
            fi
