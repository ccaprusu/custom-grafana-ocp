...
if [ "x$CITELLUS_LIVE" = "x0" ];  then
    ...
    systemctl_list_units_enabled=( "${CITELLUS_ROOT}/sos_commands/systemd/systemctl_status_--all" "${CITELLUS_ROOT}/sos_commands/systemd/systemctl_list-unit-files" "${CITELLUS_ROOT}/sos_commands/systemd/systemctl_list-unit-files" )
...
is_active(){
    if [ "x$CITELLUS_LIVE" = "x1" ]; then
        ...
        if [[ -f "${systemctl_list_units_active}" ]]; then
            grep -q "$1.* active" "${systemctl_list_units_active[@]}"
        else
...
is_enabled(){
    if [ "x$CITELLUS_LIVE" = "x1" ]; then
        ...
        if [[ -f "${systemctl_list_units_enabled}" ]]; then
            grep -q "$1.* enabled" "${systemctl_list_units_enabled[@]}"
...
is_service_running(){
    if [ "x$CITELLUS_LIVE" = "x1" ]; then
        ...
        if [[ -f "${systemctl_list_units_service_running}" ]]; then
            grep -q "$1.* running" "${systemctl_list_units_service_running[@]}"
        else
            echo "could not check for enabled service $1" >&2
            exit ${RC_SKIPPED}
        fi
